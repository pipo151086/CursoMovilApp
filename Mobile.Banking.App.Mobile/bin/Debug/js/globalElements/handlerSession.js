...
function GetContextForOTP(currentData) {
    debugger;
    if (!currentData.TieneSesion) {
        if (!SesionMovil || !SesionMovil.SessionToken)
            ResetSessionToken();
        return {
            NumeroDocumento: currentData.txtNumeroDocumento,
            SessionId: SesionMovil.SessionToken.SessionId,
            TipoDocumento: "CED"
        };
    }
    return SesionMovil.ContextoCliente;
}